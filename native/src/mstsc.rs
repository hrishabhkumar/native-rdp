...
fn tcp_from_args(server: &Server) -> RdpResult<TcpStream> {
    // TCP connection
    let addr = format!("{}:{}", server.target, server.port).parse::<SocketAddr>().map_err( |e| {
...
fn rdp_from_args<S: Read + Write>(server: &Server, stream: S) -> RdpResult<RdpClient<S>> {
    let layout = KeyboardLayout::US;
    let use_nla = !server.disable_nla;
    let width = server.width.parse().map_err(|e| {
        Error::RdpError(RdpError::new(RdpErrorKind::UnexpectedType, &format!("Cannot parse the input width argument [{}]", e)))
    })?;
    let height = server.height.parse().map_err(|e| {
    ...
    let mut rdp_connector =  Connector::new()
        .screen(width, height)
        .credentials(server.domain.to_string(), server.username.to_string(), server.password.to_string())
        .set_restricted_admin_mode(server.admin)
        .auto_logon(server.auto_logon)
        .blank_creds(server.blank_creds)
        .layout(layout)
        .check_certificate(server.check_certificate)
...
/// RDP protocol
fn window_from_args(server: &Server) -> RdpResult<Window> {
